...
- **ADT(추상 자료형) 구조 적용**: 자료구조와 관리 대상 데이터를 분리합니다.
- **인덱스 및 트리 구조를 활용한 데이터 관리**: 데이터와 정렬된 인덱스를 분리합니다. **이진 탐색 트리 (BST)** 를 활용하여 시간복잡도를 줄이고, 파일 접근 효율을 향상시킵니다. 각 노드에는 레코드의 주요 키와 파일 오프셋을 저장하여, 파일에 저장된 데이터를 빠르게 검색하고 접근할 수 있도록 합니다.
